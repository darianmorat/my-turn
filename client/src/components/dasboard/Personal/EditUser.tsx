...
import { Mail, Lock, X, Eye, EyeOff, LoaderCircle, User2 } from "lucide-react";
...
const formSchema = z.object({
   name: z
      ...
      .optional()
      .or(z.literal("")),
   role: z.string().min(1, { message: "Role es requerido" }).optional().or(z.literal("")),
   email: z
      .string()
...
export const EditUser = ({ handleModal, user }: EditUserProps) => {
   ...
   const { isLoading, editUser, users } = usePersonalStore();
   ...
   const onSubmit = async (values: z.infer<typeof formSchema>) => {
      if (values.email && values.email !== user.email) {
         ...
         if (emailExists) {
            toast.info("Este correo ya está en uso", {
               position: "top-left",
            });
            return;
         }
      ...
      if (values.email && values.email !== user.email) updates.email = values.email;
      if (values.password && values.password.trim() !== "")
         updates.password = values.password;
      if (values.role && values.role !== user.role) updates.role = values.role;
